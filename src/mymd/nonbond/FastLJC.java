...
/**
 * FastLJC class implements NonBond interface for computing Lennard-Jones 
 * plus Coulomb forces and energies using lookup tables for a faster computation 
 * speed at the cost of slight accuracy. For this class to be used, cut-off 
 * distance for coulomb and vdw sould be the same.
 *
 * @author Eunsong Choi (eunsong.choi@gmail.com)
 * @version 1.0
 */
public class FastLJC<T extends MdSystem<LJParticle>> implements NonBond<T>{

	private final double rc;
	private final int size;
	private final int numberOfDistinctTypes;
	private final LJCForceLookupTable[] forceTables;
	private final LJCEnergyLookupTable[] energyTables;

	public FastLJC(T sys){
		...
		forceTables = new LJCForceLookupTable[numberOfDistinctTypes*numberOfDistinctTypes];
		energyTables = new LJCEnergyLookupTable[numberOfDistinctTypes*numberOfDistinctTypes];
		buildTables(sys);
		...
		for ( int i = 0; i < size; i++){
			...
			for ( int j = 0; j < size; j++){ 
				...
				if ( getForceTable( iType.getNumber(), jType.getNumber()) == null ){
					...
					LJCForceLookupTable newForceTable =
					new LJCForceLookupTable(rc, C6ij, C12ij, qi*qj);
					setForceTable( iType.getNumber(), jType.getNumber(), newForceTable);

					LJCEnergyLookupTable newEnergyTable =
					new LJCEnergyLookupTable(rc, C6ij, C12ij, qi*qj);
