...
public class MdConstraint{
    ...
    public MdConstraint(int ai, int aj, int ftype, double b0){
        this.atomi = ai;
        this.atomj = aj;
        this.ftype = ftype;
        this.dij = b0;
    ...
    // convert all bonds involving hydrogen atom to constraints and add them to MdTop
    public static void convertHbondsToConstraints(GromacsSystem sys, MdTop top) {
    
        // store bond index which contains H atom (and remove these from bond list
        List<Integer> hbonds = new ArrayList<Integer>();
        int bond_index = 0;

        for ( MdBond bond : top.getBonds() ) {
            int i = bond.geti();
            int j = bond.getj();
            ...
            if ( atomi.substring(0,1).equals("H") || atomj.substring(0,1).equals("H")) {
                MdConstraint newConstraint = new MdConstraint(i, j, 1, b0);
    ...
    //public void applyConstraints(GromacsSystem sys, MdTop top, MdPrm prm, MdTraj newTraj);


    public static void applySingleConstraint(GromacsSystem sys, MdTop top, MdPrm prm, MdTraj newTraj){

        List<MdConstraint> constraints = top.getConstraints();
        ...
        for ( MdConstraint constraint : constraints ) {
            ...
            Vector rjImage_old = MdForce.minImage(box, ri_old, rj_old);
            Vector ri_new = newTraj.getPos(i);
            Vector rj_new = newTraj.getPos(j);
            Vector rjImage_new = MdForce.minImage(box, ri_new, rj_new);
            ...
            while ( Math.abs(rijSq - b*b) > tolSq && itr < maxItr) {
                ...
                rjImage_new = MdForce.minImage(box, ri_new, rj_new);
...
//System.out.println(String.format("Iteration %d : r(%d,%d)-d = %f", itr, i, j, Math.abs(rijSq - b*b)));
            ...
            MdTraj.pbc(box, ri_new);
            MdTraj.pbc(box, rj_new);
